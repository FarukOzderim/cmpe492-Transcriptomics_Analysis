function repMets=Cheng_reporterMets(model,genes,genePValues,printResults,outputFile,geneFoldChanges)
% reporterMets
...
%Remove NaN P values (Cheng fixed)
if nargin<6
	geneFoldChanges=[];
else
    geneFoldChanges = geneFoldChanges(~isnan(genePValues));
end
genes = genes(~isnan(genePValues));
genePValues = genePValues(~isnan(genePValues));
...
metNGenes(isnan(metZScores))=[];
meanZ(isnan(metZScores))=[];
stdZ(isnan(metZScores))=[];
...
mets=mets(I);
if isfield(model,'metComps')&&isfield(model,'compNames')
    metNames=cell(size(mets));
    for i = 1:length(metNames)
        indI = ismember(model.mets,mets(i));
        metNames{i} = [model.metNames{indI} '[' model.compNames{model.metComps(indI)} ']'];
    end
else
    metNames = metNames(I);
end
